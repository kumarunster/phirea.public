...
stjs.enumEntry2=function(ordinal, name, enumBody){
	this._ordinal = ordinal;
	this._name = name;
	for(var name in enumBody) {
		console.log("creating enum entry with parameter: " + name)
		this[name] = enumBody[name];
	}
};
stjs.enumEntry2.prototype.name=function(){
	//return this.enumBody.name;
	return this._name;
};
stjs.enumEntry2.prototype.ordinal=function(){
	//return this.enumBody.ordinal;
	return this._ordinal;
};
stjs.enumEntry2.prototype.toString=function(){
	//return this.enumBody.name;
	return this._name;
};
stjs.enumEntry2.prototype.equals=JavalikeEquals;

stjs.enumeration2=function(enumValues, enumFuctions, staticFunctions){
	var i = 0;
	var e = {};
	e._values = [];
	for(var enumName in enumValues){
		console.log ("log create enum, current argument is:" + enumName);
		e[enumName] = new stjs.enumEntry2(i, enumName, enumValues[enumName]);
		e._values[i] = e[enumName];
		
		for(var name in enumFuctions) {
			e[enumName][name] = enumFuctions[name];
		}
	...
	for(var name in staticFunctions){
		e[name] = staticFunctions[name];
	}
	return e;
};
